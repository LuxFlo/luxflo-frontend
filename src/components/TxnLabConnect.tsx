...
export default function TxnLabConnect() {
  ...
  const renderActiveAccount = (provider: Provider) => {
    ...
    //   const selected =
    //     options.find((option) => option.value === activeAccount.address) ||
    //     options[0];
    ...
    //         provider.setActiveAccount(selected.value)
    //       }
    //     >
    //       {options.map((option) => (
    //         <Listbox.Option
  ...
  const showSetActiveButtons = useMemo(() => {
    ...
    const areMultipleConnected =
      providers?.filter((provider) => provider.isConnected).length > 1;
    const areNoneActive = !Object.values(providers).some(
        ...
        <div className="px-5 py-3 sm:px-6 sm:py-4">
          ...
          >
            <label className="sr-only">
              Active account for {provider.metadata.name}
            </label>
  ...
  console.log("providers", providers && providers[0]);

  return (
    <div>
      {providers && providers[0].isConnected ? (
        <>
